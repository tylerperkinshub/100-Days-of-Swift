...
// Overriding methods
class DogWithOverridingMethod {
    func makeNoise() {
        print("Woof!")
    }
}

class Poodle2: DogWithOverridingMethod {
...
for _ in 1...3 {
    let person = PersonWithDeinit()
    person.printGreeting()
}


// Mutability
class SingerWithMutability {
    let name = "Taylor Swift"
}

let singerName = SingerWithMutability()
//singerName.name = "Ed Sheeran"
print(singerName.name)
...
protocol HasVatation {
    func takeVacation(days: Int)
}

protocol Employee: Payable, NeedsTraining, HasVatation { }
...
// Protocol extensions
let pythonsExtension = ["Eric", "Graham", "John", "Michael", "Terry", "Terry"]
let beatlesExtension = Set(["John", "Paul", "George", "Ringo"])
...
pythonsExtension.summarize()
beatlesExtension.summarize()


// Protocol-oriented programming
protocol IdentifiableProtoProg {
    var id: String { get set }
    func indentify()
}

extension IdentifiableProtoProg {
    func identify() {
        print("My ID is \(id)")
    }
}

struct UserProtoProg: IdentifiableProtoProg {
    func indentify() {
    }
    var id: String
}

let twostraws = UserProtoProg(id: "twostraws")
twostraws.identify()
...
var ageMissing: Int? = nil

ageMissing = 38

// Unwrapping Optionals
var nameOption: String? = nil

if let unwrapped = nameOption {
...
// Force Unwrapping
let forceUnwrapStr = "5"
let forceUnwrapNum = Int(forceUnwrapStr)

let num = Int(forceUnwrapStr)!

// Implicitly unwrapped optionals
let impUnwrappedOptional: Int! = nil


// Nil coalescing
func usernamNilCoal( for id: Int) -> String? {
    if id == 1 {
        return "Taylor Swift"
    } else {
        return nil
...
let newestUser = usernamNilCoal(for: 15) ?? "Anonymous"

// Optional chaining
let namesOptionalChaining = ["John", "Paul", "George", "Ringo"]

let beatlesChaining = namesOptionalChaining.first?.uppercased()


// Optional try
enum PasswordErrorOptional: Error {
    case obvious
}

func checkPasswordOptional(_ password: String) throws -> Bool {
    if password == "password" {
        throw PasswordErrorOptional.obvious
    }
    return true
}

do {
    try checkPasswordOptional("password")
    print("That password is good!")
} catch {
    print("You can't use that password.")
}

if let result = try? checkPasswordOptional("password") {
    print("Retult was \(result)")
} else {
    print("D'oh!")
}

try! checkPasswordOptional("sekrit")
print("okay")


// Failable initializers
let strFailableInit = "5"
let numFI = Int(strFailableInit)

struct PersonFailableInit {
    var id: String
